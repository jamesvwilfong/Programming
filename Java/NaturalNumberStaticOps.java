...
public final class NaturalNumberStaticOps {
    ...
    private static void printWithCommas(NaturalNumber n, SimpleWriter out) {
        ...
        if (temp.compareTo(thousand) < 0) {
            out.print(temp);
        } else {
            int ones = temp.divideBy10();
            int tens = temp.divideBy10();
            int huns = temp.divideBy10();
            printWithCommas(temp, out);
            out.print("," + huns + tens + ones);
    ...
    private static String toStringWithCommas(NaturalNumber n) {
        ...
        if (temp.compareTo(thousand) < 0) {
            ...
            int tens = temp.divideBy10();
            int huns = temp.divideBy10();
            return toStringWithCommas(temp) + "," + huns + tens + ones;
        }
    }
