...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        //initializes combination to the part of str1 without the overlap of str2
        String combination = str1.substring(0, str1.length() - overlap);
        //adds str2 to the end of combination and returns that String
        return combination.concat(str2);
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
         ...
         */
        //flag to tell whether str is a substring of any string already in strSet
        boolean flag = false;
        //create copy of strSet in order to use a for each loop to manipulate strSet
        Set<String> strSetCopy = new Set1L<String>();
        strSetCopy.transferFrom(strSet);
        //loop to check if str is a substring of any String already in strSet
        //and to restore strSet after being cleared by transferFrom
        for (String s : strSetCopy) {
            strSet.add(s);
            if (s.indexOf(str, 0) != -1) {
                flag = true;
            }
        }
        //if str was not found to be a substring of any String already in strSet
        //, loop to check if any String already in strSet are substrings of str
        if (flag == false) {
            for (String s : strSetCopy) {
                //if String already in strSet is substring of str, remove that
                //String from strSet
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        //initializes new set
        Set<String> a = new Set1L<String>();
        //uses addToSetAvoidingSubstrings method when parsing through the input
        //file to add all Strings that are not substrings of each other to the
        //set
        while (!input.atEOS()) {
            addToSetAvoidingSubstrings(a, input.nextLine());
        }
        //return the set
        return a;
    }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        //outputs the resulting text to the user
        out.println(text);
