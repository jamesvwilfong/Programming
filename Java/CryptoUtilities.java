...
public final class CryptoUtilities {
    ...
    public static void reduceToGCD(NaturalNumber n, NaturalNumber m) {
        ...
        //recursion stops when m = 0
        if (!m.isZero()) {
            //copy of n so that n's current value can be used for the
            //manipulation of m
            NaturalNumber n1 = new NaturalNumber2(n);
            //gives n the value of m
            n.copyFrom(m);
            //gives m the value of n1 mod m
            m.copyFrom(n1.divide(m));
            //recursive call with the updated values of n and m
            reduceToGCD(n, m);
        }
    ...
    public static boolean isEven(NaturalNumber n) {
        ...
        //creates copy of n so that n is not manipulated
        NaturalNumber temp = new NaturalNumber2(n);
        //initialize with the value of 2
        NaturalNumber two = new NaturalNumber2(2);
        //initialize to the remainder of n/2
        NaturalNumber rem = new NaturalNumber2(temp.divide(two));
        //if there is no remainder, n is even and true is returned, otherwise
        //false is returned
        if (rem.isZero()) {
            return true;
        } else {
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
         ...
         */
        //initializes copy of n
        NaturalNumber n1 = new NaturalNumber2(n);
        //stores p as an int
        int p1 = p.toInt();
        //if power is 0, set n to 1
        if (p1 == 0) {
            n.copyFrom(new NaturalNumber2(1));
            //if power is 1, perform n mod m
        } else if (p1 == 1) {
            n.copyFrom(new NaturalNumber2(n.divide(m)));
            //when power is greater than 1
        } else if (p1 > 1) {
            //recursive call to method
            powerMod(n, new NaturalNumber2(p1 / 2), m);
            //if power is odd, make this extra calculation
            if (p1 % 2 != 0) {
                n.multiply(n1);
                n.copyFrom(new NaturalNumber2(n.divide(m)));
            }
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        //initialize 2 copies of w
        NaturalNumber w1 = new NaturalNumber2(w);
        NaturalNumber w2 = new NaturalNumber2(w);

        //w1^2 mod n
        powerMod(w1, new NaturalNumber2(2), n);

        //initialize copy of n and decrement by 1
        NaturalNumber n1 = new NaturalNumber2(n);
        n1.decrement();
        //w2^(n-1) mod n
        powerMod(w2, n1, n);

        //returns true if w1=1 or w2 != 1, false otherwise
        return w1.compareTo(new NaturalNumber2(1)) == 0
                || w2.compareTo(new NaturalNumber2(1)) != 0;
    }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        //create copy of n to manipulate for the call to randomNumber
        NaturalNumber nCopy = new NaturalNumber2(n);
        //decremented 4 times so that the upper range of randomNumber will be n-4
        nCopy.decrement();
        nCopy.decrement();
        nCopy.decrement();
        nCopy.decrement();
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            ...
            //loop that creates 50 random numbers used to check the compositeness of n
            for (int i = 0; i < 50; i++) {
                //initialize random number in the range [0,n-4]
                NaturalNumber w = new NaturalNumber2(randomNumber(nCopy));
                //increment random number twice so that the range becomes [2,n-2]
                //in order to satisfy the requires clause of isWitnessToCompositeness
                w.increment();
                w.increment();
                //if any of the random numbers are a witness to the
                //compositeness of n, isPrime is set to false
    ...
    public static void generateNextLikelyPrime(NaturalNumber n) {
        ...
        //if n is even, increment to make it odd
        if (isEven(n)) {
            n.increment();
        } else {
            n.increment();
        ...
        //until n is prime increment twice to get to the next odd value
        while (!isPrime2(n)) {
            n.increment();
            n.increment();
        }
