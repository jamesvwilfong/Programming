...
public:
	...
	// randomly places the robot in any point of the room as long as it is not a barrier or door way
	void placeRobot() {
		do {
			robotY = rand() % rows;
			robotX = rand() % columns;
		...
		do {
			...
			if (directionMove == 0) {
				...
				if (robotY != 0) {
					...
					if (maze[robotY - 1][robotX] == 'd')
						escaped = true;
					//checks if movement north is not blocked
					if (maze[robotY - 1][robotX] == 'n' || maze[robotY - 1][robotX] == 'd') {
						maze[robotY][robotX] = 'n';
			...
			//movement south
				...
				if (robotY != rows - 1) {
					...
					if (maze[robotY + 1][robotX] == 'd')
						escaped = true;
					//checks if movement south is not blocked
					if (maze[robotY + 1][robotX] == 'n' || maze[robotY + 1][robotX] == 'd') {
						maze[robotY][robotX] = 'n';
						maze[robotY + 1][robotX] = 'R';
						robotY = robotY + 1;
			...
			//movement west
				...
				if (robotX != 0) {
					...
					if (maze[robotY][robotX - 1] == 'd') 
						escaped = true;
					//checks if movement west is not blocked
			...
			//movement east
				...
				if (robotX != columns - 1) {
					...
					if (maze[robotY][robotX + 1] == 'd') 
						escaped = true;
					//checks if movement east is not blocked
					if (maze[robotY][robotX + 1] == 'n' || maze[robotY][robotX + 1] == 'd') {
						maze[robotY][robotX] = 'n';
						maze[robotY][robotX + 1] = 'R';
						robotX = robotX + 1;
			...
			//movement northeast
				...
				if (robotX != columns - 1 && robotY != 0) {
					...
					if (maze[robotY-1][robotX + 1] == 'd') 
						escaped = true;
					//checks if movement northeast is not blocked 
			...
			//movement southeast
				...
				if (robotX != columns - 1 && robotY != rows - 1) {
					...
					if (maze[robotY + 1][robotX + 1] == 'd')
						escaped = true;
					//checks if movement southeast is not blocked 
					if (maze[robotY + 1][robotX + 1] == 'n' || maze[robotY + 1][robotX + 1] == 'd') {
						maze[robotY][robotX] = 'n';
						maze[robotY + 1][robotX + 1] = 'R';
						robotX = robotX + 1;
...
private:
	...
	//this allows the maze to be copy and pasted in 
	void getDimension() {
		do {
